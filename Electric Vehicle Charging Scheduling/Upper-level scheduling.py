...
def visualize_results(training_stats, p_optimal, q_optimal, env):
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig('混合策略训练过程.png', dpi=300, bbox_inches='tight')
        print("训练过程图已保存为 '混合策略训练过程.png'")
        ...
        plt.tight_layout()
        plt.savefig('混合策略最优分布.png', dpi=300, bbox_inches='tight')
        print("最优策略图已保存为 混合策略最优分布.png'")
...
def export_results(p_optimal, q_optimal, env, training_stats):
    """导出结果到CSV文件"""
    try:
        ...
        detailed_strategy_df.to_csv('混合策略_详细策略.csv', index=False, encoding='utf-8-sig')
        if not stats_df.empty:
            stats_df.to_csv('混合策略_训练统计.csv', index=False, encoding='utf-8-sig')

        print("详细策略已导出到 '混合策略_详细策略.csv'")
        if not stats_df.empty:
            print("训练统计已导出到 '混合策略_训练统计.csv'")
...
if __name__ == "__main__":
    ...
    try:
        ...
        p_optimal, q_optimal, env = analyze_final_strategies(env, station_agent, ev_agent)

        visualize_results(training_stats, p_optimal, q_optimal, env)

        export_results(p_optimal, q_optimal, env, training_stats)
